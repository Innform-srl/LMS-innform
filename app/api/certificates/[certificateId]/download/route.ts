...
export async function GET(req: Request, { params }: { params: Promise<{ certificateId: string }> }) {
    try {
            ...
            include: {
                enrollment: {
                    include: {
                        user: true,
                        course: true,
                        edition: { select: { title: true, code: true } },
                    },
                },
            },
        });
        ...
        const settings = (await db.certificateSettings.findFirst()) || {
            ...
            bodyTemplate:
                "Si certifica che {userName} ha completato con successo il corso {courseTitle} in data {completionDate}",
        };
        ...
        try {
            ...
            return new NextResponse(
                `Font Load Error: ${e instanceof Error ? e.message : "Unknown error"} | Debug: ${debugInfo.join("; ")}`,
                { status: 500 }
            );
        }
        ...
        if (regularFont && boldFont) {
            ...
            return new NextResponse(`Fonts failed to load but no error thrown? Debug: ${debugInfo.join("; ")}`, {
                status: 500,
            });
        }
        ...
        return new Promise<NextResponse>((resolve, reject) => {
            ...
            if (certificate.enrollment.edition?.title) {
                doc.moveDown(0.3);
                doc.font("Roboto")
                    .fontSize(14)
                    .fillColor("#888888")
                    .text(certificate.enrollment.edition.title, { align: "center" });
            }
            ...
            doc.fontSize(10).fillColor("#999999").text(`Certificato N: ${certificate.certificateNumber}`, 60, bottomY);
            ...
            generateQRCodeDataURL(
                `${process.env.NEXT_PUBLIC_APP_URL || "http://localhost:3000"}/verify-certificate?code=${certificate.verificationCode}`
            )
                .then((qrDataUrl) => {
