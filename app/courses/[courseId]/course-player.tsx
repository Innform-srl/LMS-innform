...
export function CoursePlayer({
    ...
    useEffect(() => {
        ...
        const meetsRequirements = isCompleted && course.minimumDuration > 0 && localTimeSpent >= course.minimumDuration;
    ...
    useEffect(() => {
        ...
        const meetsThreshold =
            course.minimumDuration > 0 && localTimeSpent >= course.minimumDuration * 60 && !enrollment.certificate;
    ...
    const timeProgress =
        course.minimumDuration > 0 ? Math.min((localTimeSpent / (course.minimumDuration * 60)) * 100, 100) : 100;
        ...
        <div className="min-h-screen bg-background text-foreground transition-colors duration-300">
                        ...
                        <div className="flex items-center gap-4 w-full md:w-auto">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-muted-foreground">
                                    {enrollment.edition ? `${enrollment.edition.title} — ` : ""}
                                    {currentModule.title}
                                </p>
                            </div>
                        </div>
                        ...
                        <div className="flex items-center gap-4 w-full md:w-auto overflow-x-auto pb-2 md:pb-0">
                            ...
                            {course.minimumDuration > 0 && (
                                <div className="flex items-center gap-3 bg-muted/50 px-4 py-2 rounded-xl border border-border min-w-[200px]">
                                    <div
                                        className={`p-2 rounded-lg ${timeProgress >= 100 ? "bg-green-500/20 text-green-400" : "bg-blue-500/20 text-blue-400"}`}
                                    >
                                        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path
                                                strokeLinecap="round"
                                    ...
                                    <div className="flex-1">
                                        <div className="flex justify-between items-end mb-1">
                                            ...
                                            <span
                                                className={`text-xs font-mono font-bold ${timeProgress >= 100 ? "text-green-400" : "text-foreground"}`}
                                            >
                            ...
                            <div className="flex items-center gap-3 bg-muted/50 px-4 py-2 rounded-xl border border-border min-w-[200px]">
                                <div
                                    className={`p-2 rounded-lg ${
                                        currentModule.minimumDuration &&
                                        currentModule.minimumDuration > 0 &&
                                        moduleTimeSpent >= currentModule.minimumDuration * 60
                                        ...
                                        <>
                                            <div className="h-1.5 bg-secondary rounded-full overflow-hidden mb-1">
                                                <div
                                                    ...
                                                    style={{
                                                        width: `${Math.min((moduleTimeSpent / (currentModule.minimumDuration * 60)) * 100, 100)}%`,
                                                    }}
                                                />
                                            </div>
                                            <div className="text-[10px] text-muted-foreground font-mono text-right">
            ...
            <div className="max-w-[1800px] mx-auto px-6 py-8 grid grid-cols-1 lg:grid-cols-4 gap-8">
                <div className="lg:col-span-3 space-y-6">
                    <div
                        className="glass border-border rounded-xl overflow-hidden shadow-2xl"
                            ...
                            <div className="bg-card min-h-[60vh] flex flex-col items-center justify-center p-8 text-center space-y-6">
                                ...
                                <div>
                                    <h2 className="text-3xl font-bold mb-2">
                                        {currentModule.liveSession.title.replace(
                                            /(\d{4})-(\d{2})-(\d{2})/,
                                            (_m, y, mo, d) => `${d}/${mo}/${y}`
                                        )}
                                    </h2>
                                    <p className="text-muted-foreground max-w-lg mx-auto">
                                        {currentModule.liveSession.description ||
                                            "Partecipa alla sessione live per completare questo modulo."}
                                    </p>
                                </div>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full max-w-lg">
                                    <div className="bg-muted/50 p-4 rounded-xl border border-border">
                                        ...
                                        <div className="font-semibold text-lg">
                                            {currentModule.liveSession.startTime
                                                ? (() => {
                                                      const d = new Date(currentModule.liveSession.startTime!);
                                    ...
                                    <div className="bg-muted/50 p-4 rounded-xl border border-border">
                                        ...
                                        <div className="font-semibold text-lg">
                                            {currentModule.liveSession.endTime
                                                ? (() => {
                                                      const d = new Date(currentModule.liveSession.endTime!);
                                                      return `${d.getDate().toString().padStart(2, "0")}/${(d.getMonth() + 1).toString().padStart(2, "0")}/${d.getFullYear()}`;
                                                  })()
                                                : "N/D"}
                                        </div>
                                    ...
                                    new Date(currentModule.liveSession.endTime) < new Date() ? (
                                        <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-lg p-4 text-sm">
                                            ...
                                            <p className="text-muted-foreground">
                                                Questa sessione live si è conclusa. Contatta il tuo amministratore se
                                                hai bisogno di accedere alla registrazione.
                                            </p>
                                        </div>
                                    ) : (
                                        <JoinSessionButton
                    ...
                    <div className="glass border-border p-6 rounded-xl">
                        <div className="flex justify-between items-start mb-4">
                            ...
                            <div className="flex items-center gap-2">
                                {!completedModules.has(currentModule.id) && (
                                    <Button onClick={handleMarkComplete} variant="outline" size="sm" className="gap-2">
                    ...
                    <div className="flex justify-between gap-4">
                        ...
                        <Button
                            ...
                            disabled={
                                currentModuleIndex < modules.length - 1 && !completedModules.has(currentModule.id)
                            }
                            ...
                            {modules.map((module, index) => {
                                ...
                                return (
                                    <div
                                        key={module.id}
                                        className="relative"
                                        data-new-module={isNewest ? module.id : undefined}
                                    >
                                        <div
                            ...
                            <div className="flex items-center gap-3 mb-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-muted-foreground">
                                        Verifica le tue conoscenze su questo modulo
                                    </p>
                                </div>
                            </div>
                    ...
                    <div className="py-4">
                        {!isCompleted && (
                            <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-lg p-4 text-sm text-yellow-600 dark:text-yellow-400">
                                ...
                                <p>
                                    Per ottenere il certificato devi ancora completare tutti i moduli e superare il quiz
                                    finale (se presente).
                                </p>
                            </div>
                        )}
